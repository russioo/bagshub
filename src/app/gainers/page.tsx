...
export default async function GainersPage() {
  ...
  const tokens = allTokens
    .filter((t: BagsToken) => t.priceChange24h > 0)
    .sort((a: BagsToken, b: BagsToken) => b.priceChange24h - a.priceChange24h)
    .slice(0, 50);
    ...
    <div>
      <div style={{ marginBottom: '48px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '16px' }}>
          Biggest gains in the last 24 hours
        </p>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-card)' }}>
                ...
                <th style={{ padding: '14px 20px', textAlign: 'right', fontSize: '12px', fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>24h Change</th>
            ...
            <tbody>
              {tokens.map((token: BagsToken, i: number) => (
                <tr key={token.mint} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '16px 20px', textAlign: 'right', fontSize: '14px', fontWeight: 500, color: 'var(--green)' }}>
